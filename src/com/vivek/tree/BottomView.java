 ...
 * The following are steps to print Bottom View of Binary Tree.
     ...
     4. Every time, we encounter a new horizontal distance or
        an existing horizontal distance put the node data for the horizontal distance as key.
        For the first time it will add to the map, next time it will replace the value.
        This will make sure that the bottom most element for that horizontal distance is present in
        the map and if you see the tree from beneath that you will see that element.
 */
public class BottomView {
    ...
    void bottomView(Node root) {
        ...
        while (!queue.isEmpty()) {
            ...
            map.put(node.horizontalDist, node.data);
