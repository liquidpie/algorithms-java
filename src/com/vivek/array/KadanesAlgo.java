...
/**
 * Find the max sum of contiguous sub-com.vivek.array.
 * For ex: for com.vivek.array [-2, -3, 4, -1, -2, 1, 5, -3], the max sum sub com.vivek.array is [4, -1, -2, 1, 5] with sum = 7.
 * 
 * The problem is solved by dynamic programming
 * initialize: max_so_far = 0
 * max_ending_here = 0
 * Loop for each element of the com.vivek.array
 * (a) max_ending_here = max_ending_here + a[i]
 * (b) if(max_ending_here < 0) max_ending_here = 0
 * (c) if(max_so_far < max_ending_here) max_so_far = max_ending_here
 * return max_so_far
 * 
 * Explanation: Simple idea of the Kadane's algorithm is to look for all positive contiguous segments of the com.vivek.array (max_ending_here is used for this). And keep track of maximum sum contiguous segment among all positive segments (max_so_far is used for this). Each time we get a positive sum compare it with max_so_far and update max_so_far if it is greater than max_so_far
