 ...
 * Write a function to sort the objects in-place on their creation sequence number in O(n) and without any extra space. For simplicity,
 ...
 * If we first find a number and then place it at its correct place, it will take us O(N2), which is not acceptable.
 * Instead, what if we iterate the array one number at a time,
 * and if the current number we are iterating is not at the correct index, we swap it with the number at its correct index.
